...
class Segments:
    def __init__(self, lens, nonzero = False):
        ...
        with tf.name_scope(name="segments") as scope:
            with tf.control_dependencies(assertions):

                self.lens = lens
                self.start_indices = tf.cumsum(self.lens, exclusive = True)
                self.segment_num = tf.size(input=self.lens)
                ...
                if nonzero:
                    ...
                    self.nonzero_indices = tf.boolean_mask(
                        tensor=tf.range(self.segment_num), mask=mask
                    )
                    self.nonzero_lens = tf.boolean_mask(tensor=self.lens, mask=mask)
                    self.nonzero_num = tf.size(input=self.nonzero_lens)
                    self.start_indices_nonzero = tf.cumsum(self.nonzero_lens, exclusive = True)

                self.empty = tf.equal(self.nonzero_num, 0)
                self.data_len = tf.reduce_sum(input_tensor=self.nonzero_lens)
                ...
                scattered = tf.cond(pred=self.empty, true_fn=scatter_empty, false_fn=scatter_nonempty)
                ...
                if nonzero: self.segment_indices = self.segment_indices_nonzero
                    ...
                    self.segment_indices = self.fill_nonzero(
                        tf.boolean_mask(tensor=tf.range(self.segment_num), mask=mask),
                    )
    ...
    def collapse_nonzero(self, data, operations = [tf.math.segment_max, tf.math.segment_sum]):
        ...
        with tf.name_scope(name="collapse_nonzero") as scope:
            res = [
    ...
    def add_zeros(self, data):
        ...
        with tf.name_scope(name="add_zeros") as scope:
    ...
    def mask_segments(self, data, mask, nonzero = False):
        ...
        with tf.name_scope(name="mask_segments") as scope:
            mask = tf.cast(mask, bool)
            data_mask = self.fill(mask)
            masked_lens = tf.boolean_mask(tensor=self.lens, mask=mask)
            masked_data = tf.boolean_mask(tensor=data, mask=data_mask)
    ...
    def mask_data(self, data, mask, nonzero = False):
        ...
        with tf.name_scope(name="mask_data") as scope:
            new_lens = self.segment_sum(tf.cast(mask, tf.int32))
            new_data = tf.boolean_mask(tensor=data, mask=tf.cast(mask, bool))
    ...
    def partition_segments(self, data, partitions, num_parts, nonzero = False):
        ...
        with tf.name_scope(name="partition_segments") as scope:
    ...
    def log_softmax(self, logits):
        with tf.name_scope(name="segments.log_softmax") as scope:
            # numeric stability
    ...
    def gather_nonzero(self, data, indices):
        ...
        with tf.name_scope(name="segments.gather") as scope:
    ...
    def entropy(self, probs = None, log_probs = None, aggregate = mean_or_zero):
        ...
        if probs is None: probs = tf.exp(log_probs)
            ...
            log_probs = tf.compat.v1.where(tf.greater(probs, 0), tf.math.log(probs),
                                 tf.zeros_like(probs))
...
class MergedSegments(Segments):
    def __init__(self, segments_list, nonzero = False):
        ...
        with tf.name_scope(name="merged_segments") as scope:
            merged_lens = tf.stack([segments.lens for segments in segments_list], axis = 1)
            Segments.__init__(self, tf.reduce_sum(input_tensor=merged_lens, axis = 1), nonzero = nonzero)
    ...
    def merge_data(self, data_list):
        with tf.name_scope(name="merge_data") as scope:
...
def unpartition_segments(seg_data_list, partitions):
    ...
    with tf.name_scope(name="unpartition_segments") as scope:
