...
from tensorflow.keras.layers import concatenate as concatenate_layer

def gather_opt(params, indices): # if params = -1, uses zero tensor

    zeros = tf.zeros(tf.shape(input=params)[1:])
...
def graph_start(graph, start_dims):
    ...
    with tf.compat.v1.variable_scope(None, "embeddings"):
        ...
        clause_emb = tf.compat.v1.get_variable("clause_emb", [3, dim_nodes])
